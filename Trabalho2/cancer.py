...
pl.xlabel('Idade',fontsize=12).applymap('{:,.3f}'.format)
pl.ylabel('Frequência', fontsize=12)
pl.xticks(range(10,120,10))
...
pl.grid(axis='x')
pl.title('Nitrogênio na Ureia -- Pacientes Idosos')
pl.xlabel('N')
pl.ylabel('Frequência')
...
pl.grid(axis='x')
pl.title('Nitrogênio na Ureia -- Pacientes Jovens')
pl.xlabel('N')
pl.ylabel('Frequência')
...
anova_table_doentes = (anova_table_doentes.replace({np.nan: 0})).applymap('{:,.3f}'.format)
print()
...
anova_table_nao_doentes = sm.stats.anova_lm(mod, typ=2)
anova_table_nao_doentes = (anova_table_nao_doentes.replace({np.nan: 0})).applymap('{:,.3f}'.format)
print()
print(anova_table_nao_doentes)
print()
...
pl.ylabel("Concentração de Nitrogênio (N)")
pl.xlabel("Idade")
pl.title("Comparação entre as regressões lineares")
pl.savefig("imgs/cancer/comparacao_regressoes.pdf")
pl.show()
